#!/usr/bin/env python3
"""
Apply Joshu xquik relay shim to the vendored last30days-skill http.py.

When JOSHU_XQUIK_MODE=relay, xquik.com requests are forwarded to the
control-plane proxy instead of using a local X-Api-Key.
...
MARKER = "# --- joshu xquik-relay shim (do not edit; re-applied by patch-last30days-xquik-relay.py) ---"

RELAY_HELPERS = '''
# --- joshu xquik-relay shim (do not edit; re-applied by patch-last30days-xquik-relay.py) ---
_XQUIK_UPSTREAM_HOST = "xquik.com"


def _xquik_relay_enabled() -> bool:
    mode = (os.environ.get("JOSHU_XQUIK_MODE") or "").strip().lower()
    if mode != "relay":
        return False
    return bool((os.environ.get("JOSHU_XQUIK_RELAY_URL") or "").strip())


def _xquik_relay_bearer() -> str:
    ...
    if not instance_id or not raw:
        raise HTTPError(
            "Xquik relay requires JOSHU_INSTANCE_ID and INSTANCE_AGENT_TOKEN",
...
def _xquik_relay_request(
    ...
    """Forward one xquik.com call via Joshu control-plane proxy."""
    relay_url = (os.environ.get("JOSHU_XQUIK_RELAY_URL") or "").strip()
    parts = urlsplit(url)
    if parts.netloc != _XQUIK_UPSTREAM_HOST:
        raise HTTPError(f"Xquik relay misconfigured for host: {parts.netloc}")
    ...
    while attempt < effective_retries:
        try:
            req = urllib.request.Request(
                ...
                headers={
                    "Authorization": f"Bearer {_xquik_relay_bearer()}",
            ...
            with urllib.request.urlopen(req, timeout=timeout) as response:
                ...
                if not isinstance(envelope, dict):
                    raise HTTPError("Xquik relay returned non-object JSON")
                ...
                if isinstance(upstream_status, int) and upstream_status >= 400:
                    err = HTTPError(
                        f"HTTP {upstream_status}: Xquik upstream error",
                        upstream_status,
                    )
            ...
            try:
                ...
                pass
            last_error = HTTPError(f"Xquik relay HTTP {e.code}: {e.reason}", e.code, body)
            ...
            last_error = HTTPError(f"Xquik relay URL error: {e}")
            if attempt < effective_retries - 1:
                time.sleep(RETRY_DELAY * (attempt + 1))
                attempt += 1
                continue
            ...
            err = HTTPError(f"Xquik relay invalid JSON: {e}", outcome_state=health.SCHEMA_DRIFT)
            _fixture_record(fixture_request, error=err, redactions=fixture_redactions)
            _raise(err)
        attempt += 1
    ...
    if last_error:
        _fixture_record(fixture_request, error=last_error, redactions=fixture_redactions)
        _raise(last_error)
    err = HTTPError("Xquik relay failed with no error details")
    _fixture_record(fixture_request, error=err, redactions=fixture_redactions)
    _raise(err)
# --- end joshu xquik-relay shim ---
'''

INTERCEPT = '''
    if _xquik_relay_enabled():
        try:
            parts = urlsplit(url)
            if parts.netloc == _XQUIK_UPSTREAM_HOST:
                return _xquik_relay_request(
                    method,
                    url,
                    headers,
                    json_data,
...
def patch_http_py(http_path: Path) -> None:
    ...
    if MARKER in text:
        ...
        end_marker = "# --- end joshu xquik-relay shim ---"
        end = text.index(end_marker) + len(end_marker)
        text = text[:start].rstrip() + "\n\n" + text[end:].lstrip()

    if "def request(" not in text:
    ...
    print(f"[patch-last30days-xquik-relay] patched {http_path}")
...
def main() -> int:
    ...
    if not http_path.is_file():
        print(f"[patch-last30days-xquik-relay] skip — missing {http_path}", file=sys.stderr)
        return 0
