...
function isVpsArozData(arozData) {
    return path.resolve(arozData) === "/var/lib/arozos";
}
function listUserDirs(usersRoot, overrideUser) {
    if (overrideUser)
        return [overrideUser];
    const names = fs
...
export function resolveJoshuFilesPaths(projectRoot = process.cwd()) {
    ...
    if (isVpsArozData(arozData) && !overrideUser) {
        console.warn("[joshu-files] JOSHU_AROZ_USER is required on VPS (AROZ_DATA=/var/lib/arozos); gbrain/Hermes file paths unavailable");
        return null;
    }
...
export function joshuFilesPathEnv(paths) {
    return {
...
export function resolveJoshuHermesWorkspaceScope(paths) {
    return {
...
//# sourceMappingURL=joshuFilesPaths.js.map