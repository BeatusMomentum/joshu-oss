...
import { BOX_SECRETS_UI_KEYS, readBoxSecretsOverrides } from "./localEnv.js";
function processEnvTrim(name) {
    return process.env[name]?.trim() ?? "";
}
function embeddingsProvider() {
...
export function isProvisionLockedSecret(name) {
    return Boolean(provisionEnvTrim(name));
}
/** Provision file → process env → Welcome box-secrets local-env. */
export function resolveBoxSecret(name, projectRoot = process.cwd()) {
...
export function isStandaloneSelfHost() {
...
export function isLlmConfigured(projectRoot = process.cwd()) {
    return Boolean(resolveBoxSecret("OPENROUTER_API_KEY", projectRoot));
}
export function isGeminiConfigured(projectRoot = process.cwd()) {
...
export function isEmbeddingsGeminiConfigured(projectRoot = process.cwd()) {
    if (resolveBoxSecret("HINDSIGHT_API_EMBEDDINGS_GEMINI_API_KEY", projectRoot))
        return true;
    if (embeddingsProvider() === "google" && isGeminiConfigured(projectRoot))
        return true;
    return false;
}
/** True when the release pins a voice-realtime image (OSS self-host default in .env.vps.example). */
export function isVoiceOffered() {
    const ref = provisionEnvTrim("JOSHU_VOICE_IMAGE_REF");
    if (!ref)
        return false;
    return !ref.includes("your-org/");
}
export function needsOpenRouterInWelcome(projectRoot = process.cwd()) {
    if (!isStandaloneSelfHost())
        return false;
    if (isProvisionLockedSecret("OPENROUTER_API_KEY"))
        return false;
    return !isLlmConfigured(projectRoot);
}
/** Standalone boxes with voice image but no Gemini key yet — collect in Welcome Connect AI. */
export function needsGeminiVoiceInWelcome(projectRoot = process.cwd()) {
    if (!isStandaloneSelfHost())
        return false;
    if (!isVoiceOffered())
        return false;
    if (isProvisionLockedSecret("GEMINI_API_KEY"))
        return false;
    return !isGeminiConfigured(projectRoot);
}
/** File brain + Hindsight need a Gemini embedding key when provider=google. */
export function needsEmbeddingsGeminiInWelcome(projectRoot = process.cwd()) {
    if (!isStandaloneSelfHost())
        return false;
    if (embeddingsProvider() !== "google")
        return false;
    if (isProvisionLockedSecret("HINDSIGHT_API_EMBEDDINGS_GEMINI_API_KEY"))
        return false;
    return !isEmbeddingsGeminiConfigured(projectRoot);
}
/** One Gemini key field in Welcome covers voice mic + google embeddings. */
export function needsGeminiMlInWelcome(projectRoot = process.cwd()) {
...
export function needsConnectAiInWelcome(projectRoot = process.cwd()) {
