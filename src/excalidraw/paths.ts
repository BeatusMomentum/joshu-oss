...
export interface CwmBoardPaths {
...
export function resolveCwmBoardPaths(filesRoot: string, requestedPath: string): CwmBoardPaths {
...
export interface CwmHandoffPath {
  readonly relativePath: string;
  readonly absolutePath: string;
}

/** Consolidation artifacts have one fixed directory and accept only a plain Markdown basename. */
export function resolveCwmHandoffPath(filesRoot: string, fileName: string): CwmHandoffPath {
  if (
    !fileName ||
...
export function defaultCwmHandoffFileName(occurredAt: string, eventId: string): string {
