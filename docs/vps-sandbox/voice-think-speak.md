...
See also [voice-realtime.md](voice-realtime.md) (service architecture). Browser mic wiring lives under the jChat / voice-realtime stack.
...
When `TWILIO_THINK_PASSWORD` is set (required for PSTN), **Joshu** gates the **entire call** — not just Hermes:

```
User speaks → transcript
           → if passphrase heard (fuzzy): unlock, disable session timers, maybe ask to restate intent
           → if wrong (clear): attempt++; at 3 → goodbye + hang up
           → if Realtime calls any tool while locked: function_output denied + ask for passphrase
           → if unlocked: normal chat / think → Hermes
```

| Concern | Detail |
| --- | --- |
| **No ungated mode** | Greeting asks for passphrase; no general chat until unlock |
| **Failed attempts** | 3 clear wrong tries hang up the call (unclear/filler does not count) |
| **Tools while locked** | **All** tool calls denied before dispatch (not just `think`) |
| **Declared tools** | PSTN declares `think` only (`PHONE_TOOL_NAMES`) — advertising `open_desktop` made the model narrate fake app opens |
| **Owner UI** | [Telephone](../telephone-arozos-app.md) app — view number / change passphrase (settings file override) |
...
| [`realtimeTools.ts`](../../packages/voice-realtime/src/realtimeTools.ts) | Tool defs; PSTN uses `PHONE_TOOL_NAMES` (`think` only) |
...
| [`twilioRealtimeSession.ts`](../../packages/voice-realtime/src/twilioRealtimeSession.ts) | Phone orchestration + passphrase gate |
...
| [`thinkPassword.ts`](../../packages/voice-realtime/src/thinkPassword.ts) | Resolve passphrase (Telephone settings.json → env) |
